...
קובצי Markdown מ־GitHub מלאים בקטעי HTML קטנים. Mowl מציג את הנפוצים שבהם כמות
שהם, במקום כתגיות טקסט: `<img>` (כולל נתיבים מקומיים, `width`/`align`, ותמונות עטופות
ב־`<div align="center">`, ב־`<p>` או ב־`<a>`), `<kbd>`, קישורים שנכתבו כ־`<a href>`,
מקטעי `<details>` / `<summary>` מתקפלים (לחיצה על הכותרת מקפלת), בלוקים של
`<div align="…">` וסימני `<!--more-->`. זו תצוגה בלבד — ה־Markdown נשמר בדיוק כפי
שכתבת, וכל השאר נשאר טקסט גולמי.

לחיצה על תמונה מציגה סרגל כלים קטן: ליישר אותה לשמאל, למרכז או לימין, לשנות את
גודלה ל־25 / 50 / 75 / 100% מהגודל המקורי, או להסיר אותה. תמונה שלא נגעו בה נשארת
Markdown רגיל (`![alt](pic.png)`); ברגע ששינית גודל או יישור היא נשמרת כ־
`<img src alt width align>`, שאותו מבינים GitHub ורוב הרנדררים האחרים — וכשמחזירים
לברירות המחדל היא חוזרת ל־Markdown רגיל.
...
אפשר גם לגרור קובץ אחד או יותר מסוג `.md` / `.markdown` / `.mdx` / `.txt` אל החלון
כדי לפתוח אותם.

משתמשים ב־`Mowl.exe` הנייד ב־Windows? במסך ההגדרות יש מקטע **System** עם כפתור
*Register* שמוסיף את Mowl לתפריט "Open with" של קובצי Markdown — לכל משתמש בנפרד,
בלי הרשאות מנהל, וניתן להסרה מלאה. (Windows לא מאפשר לתוכנה להפוך את עצמה לברירת
מחדל; בחר *Always* פעם אחת בחלון "Open with".) אם מזיזים את קובץ ההרצה, Mowl מתקן
את הרישום בהפעלה הבאה.
...
ב־Windows Mowl יכול לעדכן את עצמו. **About ← Check for updates** בודק את הגרסה
האחרונה ב־GitHub; אם יש חדשה יותר אפשר להוריד אותה, ו־Mowl מאמת את החתימה שלה מול
מפתח שמובנה באפליקציה לפני שהוא מתקין משהו. `Mowl.exe` הנייד מוחלף במקומו ומופעל
מחדש (הגרסה הקודמת נשארת לידו בשם `Mowl.exe.old` עד ההפעלה הבאה), גרסאות המתקין
מריצות את המתקין החדש, והתקנות Scoop מופנות ל־`scoop update mowl`. פעם ביום, בעת
ההפעלה, Mowl בודק בשקט ומציב נקודה על כפתור About אם יש חדש. זה הרגע היחיד שבו
Mowl פונה לאינטרנט מעצמו — אפשר לכבות זאת ב־**Settings ← System ← Check for updates
automatically** או עם `auto_check_updates = false`. ב־**Settings ← System** יש גם
כפתור *Check now*.
...
אלה ברירות המחדל. את קיצורי היישום (הכול חוץ ממקשי הלוח, הקישור וסוג הבלוק) אפשר
לשנות במסך ההגדרות — לוחצים על שדה ומקישים את הצירוף החדש — או תחת `[shortcuts]`
ב־`settings.toml`.
