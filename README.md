...
- **File associations** — set Mowl as the default app for `.md` (installer only)
- **Portable**: one `settings.toml` next to the executable, hand‑editable, live‑reloaded

Extending Mowl? Read **[ARCHITECTURE.md](ARCHITECTURE.md)** first — it maps every
...
On first run Mowl writes `settings.toml` into the folder that contains the
...
- **macOS** – Gatekeeper blocks unsigned apps: right‑click the app → *Open*, or
  run `xattr -dr com.apple.quarantine /path/to/Mowl.app`.
