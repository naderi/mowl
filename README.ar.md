...
ملفّات Markdown القادمة من GitHub مليئة بمقاطع HTML صغيرة. يعرض Mowl الشائع منها
كما هو فعلًا بدل عرضه كوسوم نصّية: `<img>` (بما في ذلك المسارات المحلّية و`width`/`align`
والصور المغلَّفة بـ `<div align="center">` أو `<p>` أو `<a>`)، و`<kbd>`، والروابط
المكتوبة بصيغة `<a href>`، وأقسام `<details>` / `<summary>` القابلة للطيّ (انقر
الملخّص لطيّها)، وكتل `<div align="…">`، وعلامات `<!--more-->`. هذا للعرض فقط —
يُحفَظ Markdown تمامًا كما كتبتَه، ويبقى كلّ ما عدا ذلك نصًّا حرفيًّا.

عند النقر على صورة يظهر شريط أدوات صغير: حاذِها إلى اليسار أو الوسط أو اليمين،
أو غيّر حجمها إلى 25 / 50 / 75 / 100٪ من حجمها الأصلي، أو أزِلها. تبقى الصورة التي
لم تُمَسّ Markdown عاديًّا (`![alt](pic.png)`)؛ وبمجرّد تغيير حجمها أو محاذاتها
تُحفَظ بصيغة `<img src alt width align>` التي يفهمها GitHub ومعظم المُصيِّرات
الأخرى — وإذا أُعيدت إلى الإعدادات الافتراضية عادت Markdown عاديًّا.
...
يمكنك أيضًا سحب ملفّ واحد أو أكثر من ملفّات `.md` / `.markdown` / `.mdx` / `.txt`
وإفلاتها على النافذة لفتحها.

تستخدم `Mowl.exe` المحمول على Windows؟ في صفحة الإعدادات قسم **System** فيه زرّ
*Register* يضيف Mowl إلى قائمة «Open with» لملفّات Markdown — لكلّ مستخدم على حدة،
دون صلاحيات المدير، ويمكن إزالته بالكامل. (لا يسمح Windows لأيّ برنامج بجعل نفسه
الافتراضيّ؛ اختر *Always* مرّة واحدة في مربّع «Open with».) إذا نقلتَ الملفّ التنفيذيّ
يُصلح Mowl التسجيل عند التشغيل التالي.
...
على Windows يمكن لـ Mowl أن يحدّث نفسه. **About ← Check for updates** يفحص أحدث إصدار
على GitHub؛ وإن وُجد إصدار أحدث يمكنك تنزيله، ويتحقّق Mowl من توقيعه بمفتاح
مضمَّن في التطبيق قبل أن يثبّت أيّ شيء. يُستبدَل `Mowl.exe` المحمول في مكانه ويُعاد
تشغيله (تبقى النسخة السابقة بجواره باسم `Mowl.exe.old` حتى التشغيل التالي)،
وتشغّل إصدارات المثبِّت المثبِّتَ الجديد، وتُوجَّه تثبيتات Scoop إلى
`scoop update mowl`. مرّة كلّ يوم عند التشغيل يفحص Mowl بصمت ويضع نقطة على زرّ About
إن وُجد جديد. هذه هي اللحظة الوحيدة التي يتّصل فيها Mowl بالإنترنت من تلقاء نفسه —
يمكن إيقافها من **Settings ← System ← Check for updates automatically** أو بـ
`auto_check_updates = false`. وفي **Settings ← System** أيضًا زرّ *Check now*.
...
هذه هي القيم الافتراضية. يمكن تغيير اختصارات التطبيق (كلّ شيء عدا مفاتيح الحافظة
والرابط ونوع الكتلة) من صفحة الإعدادات — انقر حقلًا ثم اضغط التركيبة الجديدة —
أو ضمن `[shortcuts]` في `settings.toml`.
